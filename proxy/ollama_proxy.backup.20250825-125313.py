...
async def legacy_speak(req: Request):
    """
    Smart speak:
    - If UI sends the echo line: 'I received your message: "..."',
      call Qwen with the inner quote and speak the *assistant* reply.
    - Otherwise, just speak the provided text.
    """
    try:
        data = await req.json()
    except Exception:
    ...
    # Detect the echo pattern from the UI
    echo = re.search(r'I received your message:\s*[\"“]?(.+?)[\"”]?\s*$', raw_text, flags=re.I|re.S)
    if echo:
        user_prompt = echo.group(1).strip()

        # Build /api/chat payload using configured model
        try:
            cfg = load_cfg()
        except Exception:
            cfg = {}
        ...
        try:
            ...
            # fallback: speak a simple sentence instead of code
        ...
        try:
            ...
            resp_json = None
        out_text = _extract_reply_text(resp_json)
        out_text = _clean_for_tts(out_text) or "Sorry, I couldn't generate a reply."

        # Speak assistant reply
        try:
            async with httpx.AsyncClient(timeout=30.0) as c:
                await c.post("http://127.0.0.1:11500/local/speak", json={"text": out_text})
        except Exception:
            pass
        return {"ok": True, "source": "chat+tts", "spoken": out_text}

    # Not the echo pattern: just speak the provided text (cleaned)
    clean = _clean_for_tts(raw_text)
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
            await c.post("http://127.0.0.1:11500/local/speak", json={"text": clean})
        return {"ok": True, "source": "tts-only"}
    except Exception as e:
