...
async def speak_entry(req: Request):
    """
    If the UI sends its echo line, we call the model and speak its answer.
    Otherwise we speak the provided text (cleaned).
    Returns: { ok, mode: 'chat+tts'|'tts-only', spoken }
    """
    try:
        data = await req.json()
    except Exception:
        data = {}
    raw_text = (data.get("text") or data.get("message") or data.get("content") or "").strip()
    if not raw_text:
        return JSONResponse({"error": "text required"}, status_code=400)

    # Try to pull a user prompt from the echo (or fall back to raw)
    prompt = _extract_user_prompt(raw_text)
    ...
    # If we have a prompt and a base, try chatting; else just speak the text
    if prompt and base:
        headers = {"Content-Type": "application/json"}
        ...
        try:
            ...
            try:
                ...
                resp_json = None
            out = _extract_model_text(resp_json) or (r.text or "")
            clean = _clean_for_tts(out) or "Sorry, I couldn’t generate a reply."

            # Speak the assistant reply
            try:
                async with httpx.AsyncClient(timeout=30.0) as c:
                    await c.post("http://127.0.0.1:11500/local/speak", json={"text": clean})
            except Exception:
                pass

            # tiny diag to server stdout (helps if things go off the rails)
            print(f"[speak] prompt='{prompt[:60]}' -> reply='{clean[:60]}' (status={r.status_code})")
            return {"ok": True, "mode": "chat+tts", "spoken": clean}
        except Exception as ex:
            # fall through to tts-only with a friendly line
            fallback = _clean_for_tts(f"Sorry, I couldn't contact the model. {ex}")
            try:
                async with httpx.AsyncClient(timeout=10.0) as c:
                    await c.post("http://127.0.0.1:11500/local/speak", json={"text": fallback})
            except Exception:
                pass
            print(f"[speak] chat error: {ex}")
            return JSONResponse({"ok": False, "error": f"chat forward failed: {ex}"}, status_code=502)

    # No prompt/model base → just speak cleaned raw text
    clean = _clean_for_tts(raw_text)
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
            await c.post("http://127.0.0.1:11500/local/speak", json={"text": clean})
        print(f"[speak] tts-only '{clean[:60]}'")
        return {"ok": True, "mode": "tts-only", "spoken": clean}
    except Exception as e:
