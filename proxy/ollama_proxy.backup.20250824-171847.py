...
async def tags(shape: str = "object"):
    """
    Default (no query): object with string[] models
      {
    ...
    Compat:
      - ?shape=array -> string[] of names
      - ?shape=raw   -> raw upstream JSON
    """
    try:
        cfg = load_cfg()
        base = (cfg.get("base") or "").rstrip("/")
        key  = cfg.get("apiKey") or ""
        mdl  = (cfg.get("model") or "qwen2.5-coder:32b")
    except Exception as e:
        names = [mdl]
        s = (shape or "").lower()
        if s == "array":
            return names
        if s == "raw":
            return {"models": [{"name": mdl}], "diag": {"mock": True, "error": f"config_read_failed:{e}"}}
        return {
            "models": names,
            "objects": [{"name": n} for n in names],
            "selected": mdl,
            "count": len(names),
            "diag": {"mock": True, "error": f"config_read_failed:{e}"}
        }

    async def _fetch_upstream():
        if not base:
            return None, "salad_base_not_configured"
        headers = {"Content-Type": "application/json"}
        if key:
            headers["Salad-Api-Key"] = key
        url = f"{base}/api/tags"
        async with httpx.AsyncClient(timeout=10.0) as c:
            ...
            try:
                ...
                return None, "upstream_200_nonjson"

    data, diag = None, None
    try:
        data, diag = await _fetch_upstream()
    except Exception as e:
        diag = f"upstream_failed:{e}"

    # Normalize upstream -> names[] and objects[]
    objects = []
    ...
    for m in objects:
        if isinstance(m, str):
            names.append(m)
            norm_objects.append({"name": m})
        elif isinstance(m, dict):
            n = m.get("name") or m.get("model")
            if n:
                n = str(n)
                names.append(n)
                norm_objects.append({"name": n})

    if not names:
        names = [mdl]
        norm_objects = [{"name": mdl}]

    s = (shape or "").lower()
    if s == "array":
        return names
    if s == "raw":
        return data if data is not None else {"models": [{"name": mdl}], "diag": diag}

    return {"models": names, "objects": norm_objects, "selected": mdl, "count": len(names), "diag": diag}
