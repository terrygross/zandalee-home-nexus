...
async def speak_entry(req: Request):
    ...
    prompt = _looks_like_user_prompt(raw)
    ...
    # If prompt and model base exist, do chat+tts
    if prompt and base:
        headers = {"Content-Type":"application/json"}
        ...
        try:
            ...
            try:
                ...
                js = None
            reply = _clean_for_tts(_extract_model_text(js) or (r.text or "")) or "Sorry, I couldn’t generate a reply."
            try:
                async with httpx.AsyncClient(timeout=30.0) as c:
                    await c.post("http://127.0.0.1:11500/local/speak", json={"text": reply})
            except Exception:
                pass
            print(f"[speak] prompt→reply :: '{prompt[:60]}' -> '{reply[:60]}' (status={r.status_code})")
    ...
    # Otherwise just TTS the cleaned text
    say = _clean_for_tts(raw)
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
            await c.post("http://127.0.0.1:11500/local/speak", json={"text": say})
