...
async def _speak_async(text: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
            await c.post("http://127.0.0.1:11500/local/speak", json={"text": text})
    except Exception as e:
...
# -------------------- /local/speak — unified chat-or-tts --------------------

from fastapi import Request
from fastapi.responses import JSONResponse
import re, html, httpx

def _collapse(s:str)->str: return re.sub(r"\s+"," ", s or "").strip()
...
def _clean_for_tts(text:str)->str:
    ...
    if len(s)>320: s = s[:317].rstrip()+"…"
    return s

_HEAVY = re.compile(r"[{}\[\];<>~=]{3,}")
def _codey(s:str)->bool:
    if not s: return False
    if _HEAVY.search(s): return True
    words = re.findall(r"[A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ'-]*", s)
    if len(words) < 3 and len(s) > 40: return True
    nonletters = sum(1 for ch in s if not ch.isalnum() and ch not in " .,'-?!")
    return (len(s) > 120 and nonletters/len(s) > 0.25)

_TS   = re.compile(r"^\s*\d{1,2}:\d{2}(:\d{2})?\s*$")
_LBL  = re.compile(r"^\s*(chat|voice|tts|mic|on|off|settings|gateway)\s*$", re.I)
_ECHO = re.compile(r"I received your message:\s*(.+)", re.I|re.S)
_Q    = re.compile(r'^\s*(what|why|how|who|where|when|which|can|should|could|please|tell|explain|define|write|generate|give|show)\b', re.I)

def _extract_prompt(raw:str)->str:
    if not raw: return ""
    lines = [ln.strip() for ln in str(raw).splitlines() if ln.strip()]
    lines = [ln for ln in lines if not _TS.match(ln) and not _LBL.match(ln)]
    ...
    for ln in reversed(lines):
        ...
        if m:
            ...
            if q and q[-1].strip(): return _collapse(q[-1])
            return _collapse(re.split(r'[\r\n]+', tail,1)[0]).strip('"“”')

    for ln in reversed(lines):
        if (("?" in ln) or _Q.search(ln)) and len(ln) <= 220 and not _codey(ln):
            return _collapse(ln)

    for ln in reversed(lines):
        if len(ln) <= 200 and not _codey(ln):
            return _collapse(ln)
    return ""

@app.post("/local/speak")
async def local_speak_unified(req: Request):
    """
    If text looks like a question, do chat→tts via Salad.
    Otherwise, sanitized TTS of the raw text.
    Never speaks code/JSON blobs.
    """
    try:
        data = await req.json()
    except Exception:
    ...
    do_chat = bool(prompt and base and not _codey(prompt))

    if do_chat:
        headers = {"Content-Type":"application/json"}
        if key: headers["Salad-Api-Key"] = key
        messages = [
            {"role":"system","content":"Answer in plain English, one short sentence. No code, no lists, no markdown."},
            {"role":"user","content": prompt}
        ]
        try:
            async with httpx.AsyncClient(timeout=None) as c:
                r = await c.post(f"{base}/api/chat", headers=headers, json={"model": model, "messages": messages})
            try:
                js = r.json()
            except Exception:
                js = None
            reply = (
                (js or {}).get("message",{}).get("content")
                or (js or {}).get("output_text")
                or ((js or {}).get("choices") or [{}])[0].get("message",{}).get("content")
                or (js or {}).get("content")
                or (r.text or "")
            ) or ""
            say = _clean_for_tts(reply) or "Sorry, I couldn’t generate a reply."

            try:
                async with httpx.AsyncClient(timeout=15.0) as c:
                    await c.post("http://127.0.0.1:11500/local/_speak_raw", json={"text": say})
            except Exception:
                pass

            print(f"[/local/speak] chat→tts :: prompt='{prompt[:60]}' reply='{say[:60]}' status={r.status_code}")
            return {"ok": True, "mode": "chat+tts", "spoken": say}
        except Exception as ex:
            fallback = _clean_for_tts(raw if not _codey(raw) else "Sorry, I could not contact the model.")
            try:
                async with httpx.AsyncClient(timeout=10.0) as c:
                    await c.post("http://127.0.0.1:11500/local/_speak_raw", json={"text": fallback})
            except Exception:
                pass
            print(f"[/local/speak] chat error: {ex}")
            return JSONResponse({"ok": False, "error": f"chat failed: {ex}"}, status_code=502)

    say = _clean_for_tts(raw)
    if not say or _codey(say):
        say = "Okay."
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
            await c.post("http://127.0.0.1:11500/local/_speak_raw", json={"text": say})
        print(f"[/local/speak] tts-only :: '{say[:60]}'")
        return {"ok": True, "mode": "tts-only", "spoken": say}
    except Exception as e:
        return JSONResponse({"ok": False, "error": f"speak failed: {e}"}, status_code=502)

# -------------------- TTS: RAW SAPI endpoint (internal) --------------------
...
async def local_speak_raw(req: Request):
    ...
    try:
        import asyncio
        import comtypes.client as cc  # pip install comtypes
...
# -------------------- /api/chat (LLM chat + side-effect TTS) --------------------

from fastapi import Request
from fastapi.responses import JSONResponse

def _extract_stream_text(raw_text: str) -> str:
    """
    When Salad streams JSON lines, stitch message.content pieces together.
    """
    parts = []
...
def _extract_text_for_ui(js) -> str:
    """
    Pull a plain text reply out of Salad/Ollama-ish shapes:
    - { message:{ content:"..." } }
    ...
    try:
        if isinstance(js, dict):
            if "message" in js and isinstance(js["message"], dict):
                return (js["message"].get("content") or "").strip()
            if "messages" in js and isinstance(js["messages"], list) and js["messages"]:
                last = js["messages"][-1]
                if isinstance(last, dict):
                    return (last.get("content") or "").strip()
            if isinstance(js.get("content"), str):
                return js["content"].strip()
...
async def api_chat(req: Request):
    ...
    # Ensure a model is present; default to configured one.
    if not data.get("model"):
        data["model"] = cfg.get("model") or "qwen2.5-coder:32b"

    # If stream not requested, force a single JSON reply (helps TTS and UI).
    ...
    try:
        ...
        try:
            js = r.json()
            reply_text = _extract_text_for_ui(js)
        except Exception:
            # If streaming NDJSON, stitch content
            reply_text = _extract_stream_text(raw_text)

        # SIDE EFFECT: speak the assistant's reply (best effort, non-blocking)
        if reply_text:
            asyncio.create_task(_speak_async(reply_text))

        # Return exactly what the UI expects:
        if js is not None:
            return js

        # If we only got stitched text, fabricate a minimal assistant object
...
# -------------------- Legacy UI Shims (voices/speak) — CLEAN & ANSWER --------------------

from fastapi import Request
from fastapi.responses import JSONResponse
import re, html, httpx, json as _json

def _extract_reply_text(resp):
    """
    Robustly extract assistant text from Salad/Ollama-like responses.
    """
    if resp is None:
        return ""
    if isinstance(resp, str):
        # sometimes providers send plain text
        return resp.strip()
    if isinstance(resp, dict):
        # common shapes
        c = (
            resp.get("message", {}) and resp["message"].get("content")
        ) or (
            isinstance(resp.get("choices"), list) and resp["choices"] and
        ...
        # as a last resort, try to stringify "response" or "text" fields
        c = resp.get("response") or resp.get("text") or ""
        if c: return str(c).strip()
    # don't dump JSON blobs to voice
    return ""

def _clean_for_tts(text: str) -> str:
    """
    Strip code, backticks, HTML, and links. Keep a short, speakable sentence.
    """
    if not text:
        return ""
    ...
    # prefer first sentence within ~300 chars
    m = re.match(r"(.{1,300}?[\.!\?])(\s|$)", s)
    if m:
        s = m.group(1).strip()
...
# -------------------- /speak (robust: extract echo -> chat -> clean -> TTS) --------------------
...
def _extract_user_prompt(text: str) -> str:
    """
    Pull a clean user prompt out of the UI echo or raw text:
    - Prefer the last quoted segment after 'I received your message:'
    - Otherwise, take text after the colon up to the next newline
    - Else, fall back to the whole string (trimmed)
    """
    if not text:
        return ""
    s = str(text).strip()
    # Find the echo line anywhere in the string
    m = re.search(r'I received your message:\s*(.+)', s, flags=re.I|re.S)
    if m:
        tail = m.group(1).strip()
        # Prefer the last quoted segment (handles “curly”/straight quotes)
        q = re.findall(r'[\"“](.*?)[\"”]', tail, flags=re.S)
        if q:
            cand = q[-1].strip()
            if cand:
                return cand
        # No quotes → take up to first newline / timestamp boundary
        cand = re.split(r'[\r\n]+', tail, maxsplit=1)[0].strip()
        if cand:
            # strip any trailing closing quote character if present
            cand = re.sub(r'^[\"“]|[\"”]$', '', cand).strip()
            return cand
    # No echo pattern → return the text itself
    # (strip any leading 'message:' like prefixes)
    s = re.sub(r'^\s*(message|text|content)\s*:\s*', '', s, flags=re.I).strip()
    return s
...
def _clean_for_tts(text: str) -> str:
    """
    Strip code, backticks, HTML, links; keep a short, speakable sentence.
    """
    if not text:
        return ""
    s = html.unescape(str(text))
    ...
    m = re.match(r"(.{1,300}?[\.!\?])(\s|$)", s)      # first sentence
    if m: s = m.group(1).strip()
    if len(s) > 320: s = s[:317].rstrip() + "…"
    return s
...
# -------------------- /speak (heuristic chat+tts) --------------------

from fastapi import Request
from fastapi.responses import JSONResponse
import re, html, httpx

Q_USER_START = re.compile(r'^\s*(what|why|how|who|where|when|can|should|could|please|tell|explain|define|write|generate)\b', re.I)
...
def _looks_like_user_prompt(text: str) -> str:
    """Return a user prompt if the text looks like a 'question/command' or UI echo."""
    if not text: return ""
    s = text.strip()
    m = re.search(r'I received your message:\s*(.+)', s, flags=re.I|re.S)
    if m:
    ...
    # Heuristic: short and question-like / imperative
    if len(s) <= 180 and (Q_USER_START.search(s) or "?" in s):
        return s
    return ""



# -------------------- /speak — bulletproof chat+tts --------------------
...
_TS_LINE = re.compile(r'^\s*\d{1,2}:\d{2}(:\d{2})?\s*$')  # 12:34 or 12:34:56
_LABELS = re.compile(r'^\s*(chat|voice|tts|mic|on|off|settings|gateway)\s*$', re.I)

def _guess_user_prompt(raw: str) -> str:
    """
    Extract the best user question/command from messy UI text.
    Strategy:
      - split into lines, trim; scan from bottom up
      - pick the last non-empty line that looks like a question/command
      - prefer text inside the last pair of quotes if present
    """
    if not raw: return ""
    lines = [ln.strip() for ln in str(raw).splitlines() if ln.strip()]
    lines = [ln for ln in lines if not _TS_LINE.match(ln)]             # drop timestamps
    lines = [ln for ln in lines if not _LABELS.match(ln)]              # drop labels

    # 1) direct “I received your message: …” tail
    for ln in reversed(lines):
        ...
        if m:
            ...
            if q and q[-1].strip():
                return q[-1].strip()
            # no quotes → up to first newline-ish
            return re.split(r'[\r\n]+', tail, 1)[0].strip().strip('"“”')

    # 2) pick the last line that is short-and-question-like
    for ln in reversed(lines):
        if len(ln) <= 220 and (_Q_START.search(ln) or '?' in ln):
            # prefer quoted inner content if present
            q = re.findall(r'[\"“](.*?)[\"”]', ln, flags=re.S)
            return (q[-1].strip() if q and q[-1].strip() else ln)

    # 3) otherwise, last non-empty line under a cap
    for ln in reversed(lines):
        if len(ln) <= 220:
            q = re.findall(r'[\"“](.*?)[\"”]', ln, flags=re.S)
            return (q[-1].strip() if q and q[-1].strip() else ln)

    # 4) fallback: truncate whole thing
    s = " ".join(lines).strip()
    return s[:220]
...
def _clean_for_tts(s: str) -> str:
    ...
    s = re.sub(r"`([^`]*)`", r"\1", s)                        # inline code
    s = re.sub(r"<[^>]+>", "", s)                             # HTML tags
    s = re.sub(r"\[([^\]]+)\]\([^)]+\)", r"\1", s)            # [label](url)
    s = re.sub(r"https?://\S+", "", s)                        # naked links
    s = re.sub(r"\s+", " ", s).strip()

    # Prefer first sentence, capped
    m = re.match(r"(.{1,300}?[\.!\?])(\s|$)", s)
    if m: s = m.group(1).strip()
    if len(s) > 320: s = s[:317].rstrip() + "…"

    # If looks like code noise, force a generic sentence
    if re.search(r'[{}\[\];<>=]{3,}', s):
        s = "Here’s a brief answer."
    return s

@app.post("/speak")
async def speak_entry(req: Request):
    """
    One-shot chat + TTS for the UI:
      - Accepts: {"text":"..."} (or "message"/"content")
      - Calls Salad /api/chat with cfg.model (no streaming)
      - Speaks the *assistant* reply via local TTS
      - Returns JSON with the reply (and mode)
    """
    try:
        data = await req.json()
    except Exception:
    ...
    base  = (cfg.get("base") or "").rstrip("/")
    key   = cfg.get("apiKey") or ""
    model = cfg.get("model") or "qwen2.5-coder:32b"

    # If Salad base missing, just TTS the raw text (graceful fallback)
    if not base:
        try:
            async with httpx.AsyncClient(timeout=10.0) as c:
                await c.post("http://127.0.0.1:11500/local/speak", json={"text": raw})
        except Exception as e:
            print(f"[speak] TTS-only fallback failed: {e}")
        return {"ok": True, "mode": "tts-only", "spoken": raw}
    ...
    payload = {
        "model": model,
        "stream": False,  # force non-streaming so we can parse once
        "messages": [
    ...
    try:
        ...
        # Robust reply extraction for common shapes
        try:
            js = r.json()
        except Exception as e:
            print(f"[speak] JSON parse error: {e}")
            js = None
        ...
        if isinstance(js, dict):
            reply = (
                ...
                js.get("text") or js.get("reply") or ""
            )
        if not reply:
            # Some providers return raw text in body
        ...
        try:
            async with httpx.AsyncClient(timeout=12.0) as c:
                await c.post("http://127.0.0.1:11500/local/speak", json={"text": reply})
        except Exception as e:
            print(f"[speak] TTS error: {e}")

        print(f"[speak] ok :: '{raw[:60]}' -> '{reply[:60]}'")
        return {"ok": True, "mode": "chat+tts", "spoken": reply}

    except Exception as ex:
        # On failure, log and (as last resort) speak the user text so UI stays responsive
        ...
        try:
            ...
            pass
        return JSONResponse({"ok": False, "error": f"chat failed: {ex}", "spoken": raw}, status_code=502)


# -------------------- Debug tap (non-invasive) --------------------

from fastapi import Request

@app.middleware("http")
async def _tap(request: Request, call_next):
    # Log minimal body for the critical paths; no behavior change
    try:
        path = request.url.path
        if path in ("/speak", "/local/speak", "/api/chat"):
            try:
                raw = await request.body()
                body = raw[:400].decode("utf-8", "ignore")
            except Exception:
                body = "<no body>"
            print(f"[tap] {request.method} {path} :: {body}")
    except Exception:
        pass
