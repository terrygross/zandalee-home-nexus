...
async def legacy_speak(req: Request):
    ...
    # Detect the UI echo pattern and extract the user's prompt
    m = re.match(r'^\s*I received your message:\s*[\"“]?(.+?)[\"”]?\s*$', raw_text, re.IGNORECASE | re.DOTALL)
    if m:
        user_prompt = m.group(1)

        # Build chat payload using configured model
        try:
            cfg = load_cfg()
        except Exception:
            cfg = {}
        ...
        try:
            ...
            # If chat fails, at least speak what we got asked
            try:
                async with httpx.AsyncClient(timeout=10.0) as c:
                    await c.post("http://127.0.0.1:11500/local/speak", json={"text": raw_text})
            except Exception:
                pass
            return JSONResponse({"ok": False, "error": f"chat forward failed: {ex}", "spoken": raw_text}, status_code=502)

        # Extract assistant reply text from Salad response
        out_text = ""
        try:
            resp = r.json()
        except Exception:
        ...
        if isinstance(resp, dict):
            out_text = (
                ...
                or (str(resp.get("choices",[{}])[0].get("message",{}).get("content")) if resp.get("choices") else "")
                or str(resp.get("output_text") or "")
                or str(resp.get("content") or "")
            ) or ""
        if not out_text:
            out_text = (r.text or "").strip()
        if not out_text:
            out_text = "Sorry, I couldn't generate a reply."
    ...
    # Fallback: if it wasn't the echo pattern, just pass through to /local/speak
    try:
        async with httpx.AsyncClient(timeout=10.0) as c:
            r = await c.post("http://127.0.0.1:11500/local/speak", json={"text": raw_text})
        return {"ok": 200 <= r.status_code < 300, "source": "tts-only"}
    except Exception as e:
