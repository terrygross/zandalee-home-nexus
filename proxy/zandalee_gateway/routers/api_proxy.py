...
async def chat_compat(req: Request):
    ...
    Accepts multiple shapes:
      ...
      { prompt: "..." } | { text: "..." } | { query: "..." }
    Forwards to Salad /api/chat using the configured model, and returns a
    UI-friendly response containing content in several common fields.
    """
    try:
        data = await req.json()
    except Exception:
    ...
    if not msgs:
        txt = data.get("prompt") or data.get("text") or data.get("query") or ""
        msgs = [{"role": "user", "content": str(txt)}] if txt else []
    ...
    # Normalize to a very forgiving shape
    out_text = ""
    try:
        resp = r.json()
        if isinstance(resp, dict):
            if resp.get("message", {}).get("content"):
                out_text = str(resp["message"]["content"])
            elif resp.get("output_text"):
                out_text = str(resp["output_text"])
            elif isinstance(resp.get("choices"), list) and resp["choices"]:
                msg = resp["choices"][0].get("message", {})
                if msg.get("content"):
                    out_text = str(msg["content"])
    ...
    return JSONResponse({
        ...
        "choices": [{"message": {"content": out_text}}],
        "raw_status": r.status_code
    }, status_code=200 if ok else r.status_code)
