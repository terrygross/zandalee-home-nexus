  ...
  return (
    <div className={cn("h-screen bg-lcars-black font-lcars-sans text-white flex flex-col overflow-hidden relative", className)}>
      ...
      <div className="flex-1 flex min-h-0" style={{ height: 'calc(100vh - 7.5rem)' }}>
        {/* Left Sidebar - Exact same padding as main content */}
        <div className="flex-shrink-0 z-10 w-72 md:w-80 p-4">
          <LCARSSidebar />
        </div>
        
        {/* Central Viewport - Exact same padding */}
        <main className="flex-1 p-4 min-w-0 relative overflow-hidden">
          <div className="h-full bg-lcars-black overflow-hidden">
            {children}
          </div>
        </main>
      </div>
